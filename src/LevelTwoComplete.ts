module MyGame {
	...
	export class LevelTwoComplete extends Phaser.State {
		...
		create() {
			...
			this.artPieceScoreDisplay = this.game.add.text(this.game.width/2, this.game.height/2-70, this.game.artpieces2 + '/4', style)

			this.timerDisplay = this.game.add.text(this.game.width/2, this.game.height/2-20, String(this.game.timer2), style)
